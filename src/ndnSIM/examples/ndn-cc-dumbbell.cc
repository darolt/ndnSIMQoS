...
int main (int argc, char *argv[]) {
  ...
  for (i=0; i<8; i++) { 
    ...
    consumerHelper[i]->SetAttribute ("StartTime", TimeValue (Seconds (startTimes[i])));
    consumerHelper[i]->SetAttribute ("StopTime", TimeValue (Seconds (stopTimes[i])));
