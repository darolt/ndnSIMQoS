...
Ptr<FaceContainer>
StackHelper::Install (Ptr<Node> node)
{
  ...
  for (uint32_t index=0; index < node->GetNDevices (); index++)
    {
      ...
      // hardcoded JRO
      m_disableShaper = false;
      if ((node->GetId() == 1)&&(index==0))
	m_disableShaper = true;
      if ((node->GetId() == 2)&&(index==0))
	m_disableShaper = true;
      ...
      for (std::list< std::pair<TypeId, NetDeviceFaceCreateCallback> >::const_iterator item = m_netDeviceCallbacks.begin ();
           ...
           item++)
        {
      	  NS_LOG_LOGIC ("Node " << node->GetId () << ": netdevice " << device->GetInstanceTypeId());
...
StackHelper::PointToPointNetDeviceCallback (Ptr<Node> node, Ptr<L3Protocol> ndn, Ptr<NetDevice> device) const
{
  ...
  //if ((m_shaperEnabled)&&(!m_disableShaper))
  if (m_shaperEnabled)
    {
      NS_LOG_LOGIC ("Enabled shaper on this face.");
